...
<body id="scroll_height" style="overflow-x: hidden;">
	...
	<div class="container-fluid">
		...
		@endif
			...
			<div class="" style="width: 100%; padding-top: 80px">
				<div class="avatar_box" style="width:200px;height: 115px;margin: auto; ">
					...
					@if($user->image != '')
					<img id="thumbnil" class="center rounded-circle ml-5 shadow-sm" width="115px" height="115px" src="{{asset('public/uploads/avatar/'.$user->image)}}" alt="Your avatar">
					@else
					<img class="center rounded-circle ml-5 shadow-sm" width="115px" height="115px" src="https://mdbootstrap.com/img/Photos/Avatars/avatar-10.jpg" alt="Your default avatar">
					@endif
					...
					<form id="form_edit" style="display: none" method="POST" action="{{url('/upload-image-user',[Auth::user()->id])}}" enctype="multipart/form-data">
						...
						<div class="image-upload">
							<label for="file-input">
								<i class="fas fa-camera" style="font-size:25px; position: relative;margin-left: 93px;top: -20px; color: #ff631c;"></i>
						</div>
						</label>
		...
		@endif
				...
				<div class="text">
					<p class="text-center font-weight-bold mt-3 text-dark" style="font-size: 130%">{{$user->name}}</p>
				</div>
				...
				<div class="row detail_user" style="width: 500px; margin: 0 auto;">
					...
					<div class="col-4 text-center">
						<a href="#" id="hover_font_bold" class=" text-dark">{{$user->followings()->count()}}<br> Đang theo dõi </a>
					</div>


					<div class="col-4 text-center ">
						<a href="#" id="hover_font_bold" class=" text-dark">{{$user->followers()->count()}} <br>Người theo dõi</a>
					</div>
				</div>
	...
	<div class="container">
		<div class="row m-0">
			...
			<div class="col-8 p-0">
				...
				@if(Auth()->user())
					@if(Auth()->user()->id != $user->id)
					<button type="submit" class="btn mt-2 shadow-sm action-follow" style="background: white; color: #ff631c; border: 1px solid #D3D3D3" data-id="{{ $user->id }}">
						<p class="m-0">
							<strong>
								@if(auth()->user()->isFollowing($user))
								<i class="fas fa-user-times mr-2"></i> Bỏ theo dõi
								@else
	...
	<div class="container mt-3 mb-3">
		<div class="row m-0">
				...
				<div class="p-3" style="box-shadow: 0 1px 10px 0 #D3D3D3; border: 1px solid transparent">
					...
					<div class="Following">
						...
						<div class="float-left mr-2" style="height: 45px; width: 45px;">
							<a href="{{url('user-profile/'.$follow->id)}}" alt="{{$follow->name}}">
								<img src="{{asset('public/uploads/avatar/'.$follow->image)}}" width="100%" height="100%;" style="border-radius: 50%;">
							</a>
						</div>
						@endforeach
						...
						<p class="text-uppercase" style="font-size: 80%; color: #666">Bạn chưa theo dõi ai cả</p>
			...
			<div class="col-8 p-0">
				<div class="p-3 ml-4" style="box-shadow: 0 1px 10px 0 #D3D3D3; border: 1px solid transparent">
					<p class="font-weight-bold m-0" style="font-size: 120%">Truyện của {{$user->name}}</p>
		...
		@else
		<div class="btn" style="width: 100%; background: #ff631c; color: white">
			Người này chưa có truyện nào cả!
		</div>
		@endif
