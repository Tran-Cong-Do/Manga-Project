...
<div class="row m-0">
  ...
  <div class="col-9 p-2">
    ...
    <div class="row m-0">
      ...
      <div class="col-8 p-0 pl-1 ml-3">
        ...
        <div style="width: 350px;">
          ...
          <p class="float-right m-0"><a style="color: #ff631c; font-weight: bold;" href="{{url('danh-muc/'.$truyen->danhmuctruyen->slug_danhmuc)}}">{{$truyen->danhmuctruyen->tendanhmuc}}</a>
          <p>
          <div style="clear: both"></div>
          ...
          <p class="font-weight-bold float-left m-0">Lượt xem:</p>
          <p class="float-right m-0">{{ views($truyen)->count() }}</p>
          <div style="clear: both"></div>
          ...
          <div class="float-right m-0">
            ...
            <ul class="list-inline m-0" title="Average Rating">
              @for($count = 1; $count <= 5; $count++) @php if($count<=$rating){ $color='color: #ffcc00;' ; }else{ $color='color: #ccc;' ; } @endphp <li id="{{$truyen->id}}-{{$count}}" data-index="{{$count}}" data-truyen_id="{{$truyen->id}}" data-rating="{{$rating}}" class="rating mr-1 float-left m-0" style="cursor: pointer; {{$color}} ; font-size: 20px">
                &#9733
                </li>
                @endfor
            </ul>
            @else
            <ul class="list-inline m-0" title="Average Rating">
              @for($count = 1; $count <= 5; $count++) @php if($count<=$rating){ $color='color: #ffcc00;' ; }else{ $color='color: #ccc;' ; } @endphp <li id="{{$truyen->id}}-{{$count}}" data-index="{{$count}}" data-truyen_id="{{$truyen->id}}" data-rating="{{$rating}}" class="rating mr-1 float-left m-0" style="pointer-events:none; {{$color}} ; font-size: 20px">
                &#9733
                </li>
                @endfor
            </ul>
          ...
          @if($chapter_dau)
          <a href="{{url('xem-chapter/'.$chapter_dau->truyen->slug_truyen.'/'.$chapter_dau->slug_chapter)}}">
            <div class="btn pt-1 pb-1 font-weight-bold m-0 float-left mr-2" style=" background: #ff631c; color: white; border-radius: 8px"><i class="fas fa-book-open mr-2"></i>Bắt đầu đọc</div>
          </a>
          @elseif($chaptertranh_dau)
          <a href="{{url('xem-chapter-tranh/'.$chaptertranh_dau->truyen->slug_truyen.'/'.$chaptertranh_dau->slug_chaptertranh)}}">
            <div class="btn pt-1 pb-1 font-weight-bold m-0 float-left mr-2" style=" background: #ff631c; color: white; border-radius: 8px"><i class="fas fa-book-open mr-2"></i>Bắt đầu đọc</div>
          </a>
          @else
          <a>
            <div class="btn pt-1 pb-1 font-weight-bold m-0 mb-2" style="background: #DC143C; color: white; border-radius: 8px;">Hiện tại chưa có chương mới...</div>
          </a>
          @endif

          @if(Auth::user())
          <!-- <div class="btn btn-thich_truyen pt-1 pb-1 font-weight-bold" style="background: #ff631c;  border-radius: 8px; color: white">
          <i class="fa fa-heart mr-2" aria-hidden="true"></i>Yêu thích
          </div> -->


          <div class="panel panel-info" data-id="{{ $truyen->id }}">

            <div class="panel-footer font-weight-bold btn pt-1 pb-1 " style="background: #ff631c;  border-radius: 8px;">
             <a href="#" title="Nature Portfolio" class="m-0"></a>
              <span class="pull-right">
                <span class="like-btn m-0" style="color: white">
                  <div id="like{{$truyen->id}}" class="btn-thich {{ $truyen->favoriters()->count() > 0  ? 'like-post' : '' }}">
                  @if(auth()->user()->hasFavorited($truyen))
                  <i class="fa fa-heart mr-2" aria-hidden="true" style="color: red"></i>Bỏ yêu thích
                  @else
          ...
          @if($chapter_dau)
          <a href="{{url('xem-chapter/'.$chapter_cuoi->truyen->slug_truyen.'/'.$chapter_cuoi->slug_chapter)}}">
            <div class="btn mt-3 pt-1 pb-1 font-weight-bold m-0" style=" background: #ff631c; color: white; border-radius: 8px">Đọc chương mới nhất</div>
          </a>
          @elseif($chaptertranh_dau)
          <a href="{{url('xem-chapter-tranh/'.$chaptertranh_cuoi->truyen->slug_truyen.'/'.$chaptertranh_cuoi->slug_chaptertranh)}}">
            <div class="btn mt-3 pt-1 pb-1 font-weight-bold m-0" style=" background: #ff631c; color: white; border-radius: 8px">Đọc chương mới nhất</div>
          </a>
          @else
          <div></div>
    ...
    <div>
      ...
      <a href="{{url('the-loai/'.$thuocloai->slug_theloai)}}">
        <div class="btn font-weight-bold" style=" background: #ff631c; color: white;padding: 0px 5px 0px 5px;">{{$thuocloai->tentheloai}}</div>
      </a>
      @endforeach
    </div>
  ...
  <div class="col-3 p-0">
    ...
    <div>
      ...
      @php 
        $count_fav = count($truyenyeuthich);
      @endphp
      @if($count_fav > 0)
      ...
      @foreach($truyenyeuthich as $key => $tr)
        <div class="mb-2">
          ...
          <div class="row m-0 text-left ">
            ...
            <div class="post_story_box  mb-2" style=" width: 100%;">
              <div class=" shadow-sm" style="width: 100%;border-radius: 8px;">
                <a href="{{url('xem-truyen/'.$tr->favoriteable->slug_truyen)}}" class="text-dark">
                  <div class="image_book_box float-left" style="width: 15%;">
                    <img class="card-img-top" src="{{asset('public/uploads/truyen/'.$tr->favoriteable->hinhanh)}}" width="50px" height="70px" style="border-radius: 8px; display:block;">
                  </div>
                </a>
                <div class="image_book_box pl-2 float-right" style="width: 85%;">

                  <a class="text-dark" href="{{url('xem-truyen/'.$tr->favoriteable->slug_truyen)}}">
                    <p class="title_story mb-0 font-weight-bold resomer1" style="font-size: 110%;">{{$tr->favoriteable->tentruyen}}</p>

                    <p class="m-0" style="font-size: 100%">Tác giả: {{$tr->favoriteable->tacgia}}</p>

                  </a>
                  <form action="{{route('favorite.delete',[ $tr->id])}}" method="POST">
                    @method('DELETE')
                    @csrf
      ...
      @else
      <p class="text-center">Bạn chưa có truyện yêu thích nào</p>
      @endif
    </div>
    @else
    <div>
...
<form method="POST" action="{{ route('comment.store'   ) }}">
  ...
  <div class="container mb-3 mt-3">
    ...
    <div class="media mt-3 shadow-textarea">
      ...
      <img class="d-flex rounded-circle avatar z-depth-1-half mr-3 shadow-sm" width="45px" height="65px" src="{{asset('public/uploads/avatar/'.Auth::user()->image)}}" alt="Avatar">
      </a>
      @else
      <a href="{{url('user-profile/'.Auth::user()->id)}}">
      <img class="d-flex rounded-circle avatar z-depth-1-half mr-3 shadow-sm" width="45px" height="65px" src="https://mdbootstrap.com/img/Photos/Avatars/avatar-10.jpg" alt="Avatar">
      </a>
      @endif
      <div class="media-body">
        <h5 class="m-0 mb-1 font-weight-bold blue-text"><a class="text-dark" href="{{url('user-profile/'.Auth::user()->id)}}" title="{{Auth::user()->name}}">{{Auth::user()->name}}</a></h5>
        <div class="form-group basic-textarea rounded-corners orange-border">
          <textarea class="form-control z-depth-1 shadow-sm" id="exampleFormControlTextarea345" rows="3" placeholder="Viết bình luận của bạn ở đây..." name="body" style="resize: none;"></textarea>
